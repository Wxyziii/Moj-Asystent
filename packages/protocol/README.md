...
## Current implemented events (1.2)
...
`assistant.response.completed`, `model.status.changed` and `system.error`.
...
Protocol 1.2 capability requirement as the event stream and never expose local
model paths.

Versions are exact capabilities. The current client and core both require
`1.2`; a `1.1` client rejects it. A new minor version is accepted only after it
