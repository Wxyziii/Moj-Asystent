...
The public onboarding/status payload never exposes the local model path. The local API requires the per-launch desktop credential and Protocol 1.2 on every request; UUIDs, request bodies and base64 audio are size-limited and schema-validated.
