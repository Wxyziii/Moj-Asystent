...
from moj_asystent_core.protocol import parse_event
...
def hello() -> dict:
    return {
        ...
        "correlation_id": None,
        "type": "client.hello",
        "payload": {"client_id": "integration", "protocol_version": "1.0"},
    }
...
async def running_server(port: int = 0):
    ...
    server = create_server(CoreSettings(port=actual_port))
...
async def test_real_websocket_broadcast_disconnect_shutdown_and_restart() -> None:
    async with running_server() as (server, port, server_task):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"http://127.0.0.1:{port}/health", headers={"Origin": "http://localhost:1420"}
            )
            assert response.headers["access-control-allow-origin"] == "http://localhost:1420"
        async with connect(url, origin="http://localhost:1420") as first, connect(url) as second:
            hello_ids = []
    ...
    async with running_server(port) as (_, same_port, _):
        ...
        async with connect(f"ws://127.0.0.1:{port}/ws") as connection:
...
async def test_lifespan_shutdown_cancels_pending_handshake_and_sender() -> None:
    async with (
        ...
        connect(f"ws://127.0.0.1:{port}/ws") as pending,
        connect(f"ws://127.0.0.1:{port}/ws") as ready,
    ):
